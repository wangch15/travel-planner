...
(() => {
  ...
  function renderProvider(id){
    ...
    const actions=el('div',undefined,'settings-actions'),connect=el('button',value.state==='unavailable'?'前往安裝':value.state==='waiting-login'?'重新確認':'連接');connect.dataset.providerAction='connect';connect.disabled=busy.has(id);connect.hidden=value.state==='connected';connect.setAttribute('aria-describedby',description.id);connect.onclick=()=>value.state==='unavailable'?(settingTab('tools'),window.openToolSetup?.(id)):runAccountAction(id,value.state==='waiting-login'?'check':'login');
