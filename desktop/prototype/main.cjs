...
async function createWindow({ pickDirectory,pickReferences,saveArchivePath,pickArchivePath,pickProjectParent,makeToolSupport=(dir,options)=>new ToolSupport(dir,options),makeTrash=()=>new TripTrashService(),makeProvider=(id,directory,options)=>require('./providers/index.cjs').createProvider(id,directory,options),makeProjectSetup=()=>new ProjectSetupService(),fetchReference=fetchPublicReference,referenceOptions,makeResearch=options=>createResearchTools(options),
  ...
  const win = new BrowserWindow({
    ...
    title: 'Travel Planner', backgroundColor: nativeTheme.shouldUseDarkColors ? '#191919' : '#ffffff',
  ...
  const accountAction = action => async event => {
    ...
    try { if(requestedProvider==='gemini')throw Object.assign(Error('PROVIDER_UNAVAILABLE'),{code:'PROVIDER_UNAVAILABLE'});return { ok: true, account: {...await action(),provider:requestedProvider,capabilities} }; }
      ...
      const messages = {
        ...
        CLI_MISSING:'尚未找到所選 AI 工具，請到「設定 → 帳號連線」按「安裝」。',PROVIDER_UNAVAILABLE:'Gemini 連線暫停；既有對話仍保留，請改用 Codex 或 Claude。',EXTERNAL_PROVIDER_POLICY:'這個工具的公司管理原則與 App 隔離設定不相容，原設定保持不變。Claude 目前支援 Pro／Max 帳號。',UNSUPPORTED_PROVIDER_VERSION:'舊版 Gemini CLI 接法無法驗證目前工具的安全限制；不會只因版本號而忽略限制。',SUBSCRIPTION_LOGIN_REQUIRED:'請使用這個服務的官方帳號登入；App 不會自動改用付費 API。',PROVIDER_AUTH_INVALID:'登入資料需要重新核對，請使用更多選單重新登入。',
      ...
      return { ok: false, code:error.code==='ENOENT'?'CLI_MISSING':error.code||error.message, message: messages[error.code] || messages[error.message] || 'AI 連接未完成，已保存的登入仍保留。請重新檢查，或到「設定 → App 更新 → 疑難排解」查看狀態。' };
    }
  };
  ...
  // 這輪確定結束（沒有東西在遠端繼續跑）的錯誤：不算「結果不明」，可以直接重送。
  const SETTLED_FAILURES=['LOGIN_REQUIRED','QUOTA_UNAVAILABLE','QUOTA_EXHAUSTED','POLICY_MISMATCH','MODEL_UNAVAILABLE','INVALID_DAY','EFFORT_UNAVAILABLE','AI_USAGE_LIMIT','AI_SERVICE_BUSY','AI_TIMEOUT'];
  const workflowFailure = error => {
    const name=({codex:'ChatGPT（Codex）',claude:'Claude',gemini:'Gemini'})[providerId]||'AI';
    const messages = {
      ...
      EFFORT_UNAVAILABLE:'這個模型不支援選的思考級別，請重新選一個。',
      ...
      LOGIN_REQUIRED:`${name} 需要重新登入，這輪沒有完成，行程沒有被修改。登入後按「重送這則」。`, QUOTA_UNAVAILABLE:'目前無法確認一般額度可用，這次未送出模型請求。',
      POLICY_MISMATCH:`${name} 的執行設定與 App 的安全要求不符，已停止送出。`, AI_BUSY:'已有一個 AI 工作正在執行。',
      AI_USAGE_LIMIT:`${name} 的使用額度暫時用完了，這輪沒有完成，行程沒有被修改。等額度恢復後按「重送這則」，或從對話選單改用其他 AI 開新對話。`,
  ...
  async function runAI(input,target,{automatic=false}={}){
    ...
    try{
      ...
      const settledFailure=e.settled===true||SETTLED_FAILURES.includes(e.code);
      if(job){try{await jobs.fail(target,job.id,e.code,e.limits,{settled:settledFailure});if(automatic&&['QUOTA_UNAVAILABLE','QUOTA_EXHAUSTED'].includes(e.code)&&job.input.autoAttempts<3)await jobs.wait(target,true);}catch{}}
      if(requestSaved){try{const uncertain=!completedAnswer&&!settledFailure;const updated=await conversations.update(target,s=>{
        ...
        else if(e.code==='AI_CANCELED'&&!settledFailure&&s.job?.id===job.id)Object.assign(s.job,{status:'unknown',autoResume:false,reason:'connection-interrupted'});
        if((e.code==='QUOTA_EXHAUSTED'||e.settled===true)&&e.turnId&&s.thread)s.thread.lastTurnId=e.turnId;if(completedAnswer&&s.thread)s.thread.lastTurnId=completedAnswer.turnId;
  ...
  function featureMessage(error){
    ...
    return texts[error.code]||userFacingMessage(error)||workflowFailure(error).message;
  }
  ...
  feature('provider-account-action',async input=>{
    ...
    }catch(error){const code=error.code||error.message;const messages={CLI_MISSING:'尚未安裝，按「安裝」就能裝好。',ENOENT:'尚未安裝，按「安裝」就能裝好。',UNSUPPORTED_PROVIDER_VERSION:'舊版 Gemini CLI 接法無法驗證目前工具的安全限制。',EXTERNAL_PROVIDER_POLICY:'帳號或公司管理原則與這版隔離設定不相容。',SUBSCRIPTION_LOGIN_REQUIRED:'請以官方訂閱帳號登入。'};return {account:{...providerView(id),state:['CLI_MISSING','ENOENT'].includes(code)?'unavailable':'error',message:messages[code]||'連線未完成，請重新檢查或重試。'}};}
  ...
  // 其他工具暫存（git add）過的改動會擋備份：取消暫存只放回一般修改，檔案內容不變。
  feature('backup-unstage',async()=>{if(!currentProject)throw Error('PROJECT_REQUIRED');await backup.unstage(currentProject.root);return {};},{exclusive:true});
  feature('backup-project-prepare',async()=>{if(!currentProject)throw Error('PROJECT_REQUIRED');await readyForBackup(currentProject.root);return {preparation:await backup.prepare({root:currentProject.root,slug:null})};},{exclusive:true});
  // 不連網的「尚未備份」提示用；失敗時回報未知，不阻擋其他操作。
  feature('backup-status',async input=>{if(!currentProject)return {status:null};const slug=input?.slug&&currentProject.trips.some(t=>t.slug===input.slug)?input.slug:null;try{return {status:await backup.localStatus(currentProject.root,slug)};}catch(error){return {status:null,error:featureMessage(error)};}});
  // 發布前如果還有沒備份的修改，先自動 commit + push 到私人 GitHub（備份本身仍會重新確認目的地是私人專案）。
  // 讀不到備份狀態時不能當成「不用備份」然後直接發布：讓錯誤照常回報。
  const needsBackup=async target=>{const st=await backup.localStatus(target.root,target.slug);return st.neverBackedUp||st.pendingFiles>0||st.unpushedCommits>0?{pendingFiles:st.pendingFiles}:null;};
  ...
  feature('publish-confirm',async input=>{if(proposals.pending||materialization)throw Object.assign(Error('PREVIEW_REQUIRED'),{code:'PREVIEW_REQUIRED'});
    let backedUp=null;
    if(input.slug){const target=selectedTarget(input);if(await needsBackup(target)){await readyForBackup(target.root);const prep=await backup.prepare({root:target.root,slug:target.slug});backedUp=await backup.confirm(prep.token);
