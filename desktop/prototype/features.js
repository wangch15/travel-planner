...
(() => {
  ...
  async function prepareTool(id){const {preparation:p}=await api('tool-prepare',{id});toolPlan=p;$('tool-setup-title').textContent=p.method==='background-update'?'更新 '+(toolNames[id]||id):'準備 '+(toolNames[id]||id);$('tool-setup-steps').replaceChildren(...p.steps.map(step=>el('li',step)));$('tool-command').textContent=p.commandPreview||'';$('tool-command-details').hidden=!p.commandPreview;$('tool-setup-status').textContent='確認後才會開始，不會要求你把密碼貼進 App。';$('tool-setup-confirm').hidden=false;$('tool-setup-confirm').textContent=p.method==='terminal'?'開啟安裝視窗':p.method==='background-update'?'更新':p.method==='bundled'?'檢查內建工具':p.method==='system-dialog'?'開啟 Apple 安裝視窗':'下載並安裝';$('tool-setup-dialog').showModal();}
  ...
  $('tool-setup-confirm').onclick=()=>action('tool-setup-confirm',async()=>{if(!toolPlan)return;const token=toolPlan.token;toolPlan=null;const {result}=await api('tool-install',{token,confirmed:true});$('tool-setup-status').textContent=result.message||({installed:'安裝完成，已驗證版本。',updated:'更新完成，已驗證版本。','terminal-opened':'請依系統安裝視窗完成，然後回到這裡重新檢查。','dialog-opened':'已開啟 Apple 的安裝視窗，請按「安裝」；完成後回到這裡重新檢查。',ready:'工具已就緒，可回設定連接帳號。','repair-app':'內建工具不完整，請重新安裝 App。'})[result.state]||'請重新檢查工具狀態。';$('tool-setup-confirm').hidden=true;await environment();if(['installed','updated'].includes(result.state))await window.recheckProvider?.(result.tool);});
