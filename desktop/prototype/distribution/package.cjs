...
function packageFilter(relative){const p=relative.replaceAll('\\','/');if(/(^|\/)(node_modules|tests|test|\.git|\.local|\.cache|trips)(\/|$)/.test(p)||/smoke\.cjs$|\.test\.[cm]?js$|\.map$/.test(p))return false;if(p.startsWith('desktop/prototype/distribution'))return false;if(p.startsWith('desktop/prototype/')&&/\.(md|log)$/.test(p))return false;return ['src','public','scripts/lib','scripts/templates','packages/engine','desktop/prototype','docs/schema'].some(prefix=>p===prefix||p.startsWith(prefix+'/'))||['desktop/spikes/inspect-project.cjs','.githooks/pre-push','scripts/pre-push.js'].includes(p)||/^scripts\/(check|build|ship|unship|adopt-deploy|migrate|photos)\.js$/.test(p);}
...
async function copyRuntime(destination,{root=ROOT,version=require('../package.json').version}={}){
  ...
  for(const relative of ['src','public','scripts/lib','scripts/templates','packages/engine','desktop/prototype','desktop/spikes/inspect-project.cjs','.githooks/pre-push','scripts/pre-push.js','docs/schema',...['check','build','ship','unship','adopt-deploy','migrate','photos'].map(n=>'scripts/'+n+'.js')]){try{await fs.cp(path.join(root,relative),path.join(destination,relative),{recursive:true,filter:source=>packageFilter(path.relative(root,source)),dereference:false});}catch(e){if(e.code!=='ENOENT')throw e;}}
