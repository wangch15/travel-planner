...
AI 助手裝好就能連，舊版 Claude 在 App 裡按一下就更新，不用再開終端機。

- **裝好 Codex／Claude 後按鈕會變「連接」**：以前安裝其實成功了，但帳號連線那一列沒有重新確認，一直停在「安裝…」，只好重複安裝好幾次。
- **舊版 Claude 在 App 裡直接更新**：電腦上已經有 Claude Code 但太舊時，按鈕改成「更新…」，App 會用官方更新功能更新它實際在用的那一份（不行就改用官方安裝程式），再自動重新確認連線。首次設定遇到舊版也會自動更新再登入，不會空等。
- **不再一律說「公司管理原則不相容」**：安全檢查沒跑起來、Claude 沒回報方案（多半是版本太舊）、真的是公司方案，現在各自說明；這幾種情況仍然不會連上。Windows 的安全檢查改用 PowerShell 的完整路徑。
- 背景安裝工具的視窗按鈕改為「下載並安裝」（原本誤寫成「檢查內建工具」）。
...
- **預覽失敗時說出原因**：以前只有一句「無法產生預覽」，輸入框也不能用。現在會說出是哪個檔案、有幾處沒通過檢查（例如「每日行程與地點（data.js）1 處」），逐項問題收在「看詳細問題」。
- **依原因給下一步**：資料格式太舊→「更新旅程資料夾…」；比 App 新→「前往 App 更新」；上次備份那一版實際檢查過能正常顯示→「回到上次備份…」（先列出會改哪些檔案，確認後才動）；上次備份也壞掉就不給這顆按鈕並說明原因。
- **回報給開發者**：確認視窗先顯示完整內容與公開目的地，按「送出回報」才用 App 登入的 GitHub 在模板 repo 開 issue；沒登入時可在瀏覽器開啟已填好的回報頁。內容只有版本、錯誤代碼、檔案種類與問題類型，不含行程內容、地點名稱、網址或電腦上的位置。
- **修正**：AI 改完檔案、預覽還在背景重建時，按「回到修改前」「全部不要」或打開版本紀錄會被說成「已有一個 AI 工作正在執行」而拒絕；現在會等重建完成再執行。

## 0.1.6（2026-09-25）

聊天不再卡住、設定頁照「要做的事」重新整理。

- **AI 出錯不再鎖住聊天**：以前大多數錯誤都被當成「結果不明」，送出鍵停用，Claude 更是只能「重新開始」。現在確定結束的失敗（Claude 每輪結束、Codex 伺服器回報結束）直接出現「這輪沒有完成」卡片，可以「重送這則」或「帶回輸入框」；重送不會蓋掉輸入框裡還沒送出的字。
- **錯誤說人話**：Claude 的錯誤分成登入過期、額度用完、服務忙線，附「重新登入」；訊息寫出實際使用的 AI，不再寫死「Codex」。沒預料到的錯誤只給一個可回報的代碼，不顯示程式內部訊息。
- **慢但正常的回覆不再被切斷**：改成「多久沒有新進度」才停止，另有總上限。Codex 按「停止回覆」後 10 秒內沒確認就先放開畫面。
- **不用先打開預覽才能送出**：打開旅程就在背景檢查行程；面板沒打開時不會載入預覽，發布前「看過預覽」的確認照舊。
- **輸入框 ↑／↓ 找回送出過的訊息**：還沒送出的草稿翻回底會還原，翻到的舊訊息改過也會保留。
- **設定頁重新整理**：分成「旅程與帳號」（帳號連線、我的旅程資料、備份與分享、AI 可以查的資料）與「App」（外觀、App 更新、關於），底部「重新走一次設定」。
  - 帳號連線：GitHub、AI、Cloudflare 在同一頁；「新對話預設使用／使用模型／思考級別」收進「進階」。
  - 備份與分享：不再分頁；需要登入 GitHub 或 Cloudflare 時就地登入，代碼與進度也顯示在原地。
  - 我的旅程資料：先說明什麼是旅程資料夾，三種取得方式並排，資料夾位置收進「詳細資訊」。
  - App 更新：工具清單收進「疑難排解」。
- **設定引導**：還是在第一次使用的引導裡做；之後缺東西（例如 AI 需要重新登入）時，聊天上方出現「還差一步」，按下回到引導的那一步。引導每一步都能「稍後再設定」。
- **用詞**：「專案」改成「旅程資料夾」或「GitHub 私人備份」（「更新專案」→「更新旅程資料夾」）；推送→上傳、思考強度→思考級別；移除「唯讀」「確認才保存」「原專案未修改」等和實際行為相反的舊說明。
