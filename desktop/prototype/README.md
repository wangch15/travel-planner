...
Codex 與 Claude Code 使用本機已安裝的指令、App 專屬登入設定；工具更新版號不會單獨造成拒絕，實際連線及送出仍須通過協定與權限核對。工具頁顯示的版號只供診斷，並非准用清單；安裝引導仍須人確認，不會自動升降版。Google 目前仍是舊的 Gemini CLI 接法，尚未換成 `agy`：官方 Antigravity CLI 使用系統鑰匙圈，獨立帳號的隔離方法未經證實之前，不會讓 App 以你在終端機的主要帳號代替登入。詳見 `docs/plans/2026-09-23-desktop-native-cli-profiles.md`。
...
聊天與新對話預設的服務選單只列出目前已連接的 AI；未連接的服務仍可在設定中的服務連線區塊完成授權。已開始的對話以純文字呈現鎖定的服務名稱，不顯示下拉箭頭。Codex 新對話的預設思考強度為明確的「中」（模型支援時），可在 AI 設定改成其他支援值；模型不支援原強度時，選擇該模型會改用可用強度並保存。既有對話的強度不會被新預設覆寫；舊對話若選了模型預設，介面只在模型有提供確切值時顯示該值，不猜測未知強度。Claude Code／Gemini 不顯示思考強度設定。
