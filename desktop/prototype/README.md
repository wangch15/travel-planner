...
第一個安裝用 DMG 定為 **0.1.0**（之前的 0.2.x／0.3.x 是開發階段的內部編號，保留在下方紀錄）。規則：每個進 main 的桌面改動 +0.0.1；準備發新的 DMG 時 +0.1.0；穩定後才進 1.0.0。版號只維護在 `desktop/prototype/package.json`，側欄品牌後方與「關於」頁都從這裡讀。
