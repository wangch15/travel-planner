...
- **焦點與回饋**：一般 `:focus-visible` 為 2px focus 色外框、向外偏移 3px。composer 以容器的 1px `:focus-within` 外框替代 textarea 外框；主題 radio 的可見選項有 2px 外框。停用按鈕透明度為 0.5；會等結果的按鈕執行中不變淡，寬度不變，改顯示轉圈與進行中的動詞（`busy.js`），執行中不能重按。通知顯示約 6.5 秒，表單錯誤保留在表單內。
...
「備份與發布」是單一設定目的地，以可用鍵盤操作的分頁保留私人與公開操作的不同語意。設定頁只放狀態、帳號與進階；實際的備份、回到上次備份、發布與網站歸屬一律在共用步驟燈箱（`sync-flow.js`）裡依「檢查 → 核對 → 完成」進行，頁首、聊天卡片與設定頁都打開同一個燈箱。燈箱主要按鈕在右下、取消在其左、危險動作以文字按鈕放左下；推送或發布中鎖住取消、關閉與 Escape。按鈕只有一般 32px 與小型 28px 兩種高度。群組標題左、主要檢查操作右；工具版本、狀態接在名稱後。統一32px控制高度及8px action gap，縮短設定列垂直間距，860px窄視窗仍保持可讀且不水平溢出。
