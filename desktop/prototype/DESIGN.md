...
- **聊天**：使用者訊息在右側灰底對話框內，助手回覆靠左且不包同樣底框。composer 保留草稿；Enter 傳送、Shift+Enter 換行，輸入法組字時不送出。既有旅程通過預覽驗證且連接帳號／模型後可送出。聊天輸入框內提供模型選單，與設定頁同步。範圍預設「不指定 · 整體討論」，明示這輪只給建議；選定一天才產生可保存的提案。
...
- **品牌資產**：兩份使用者提供的 v2 透明 SVG 保持原始檔案內容。明確亮／暗模式選相應 v2 SVG，跟隨系統使用生效主題對應的 v2 SVG；favicon 使用同源匯出的背景圖示。manifest 與 touch icon 使用對應明暗 PNG，manifest 的 purpose 為 `any`，不宣稱 maskable。桌面圖示由原始資產匯出，不能為配合灰階介面改色或重畫。
...
macOS Dock／ICNS 使用專用圓角背景與透明外緣；UI 的 v2 原始透明 SVG 不變。平台匯出參數見 assets/brand/README.md。
