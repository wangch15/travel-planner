...
function decodeAnswer(text,{mode,threadId,turnId,model}){
  ...
  if(mode==='research'){
    ...
    if(a.unresolved.some(v=>typeof v!=='string'||v.length>2000))throw error('AI_OUTPUT_INVALID');
    return {...base,research:true,sources,unresolved:a.unresolved,feasibility:a.feasibility};
  }
...
class CodexEditor {
  ...
  async generate({ snapshot, dayId, text, model, onProgress = () => {}, thread = null, onThread = async () => {}, lastOutcome = '尚未產生提案。', mode:requestedMode, effort, attachments=[], planningDraft=null, handoff=null, requestId=null, onTurn=async()=>{}, onDelta=()=>{} }) {
    ...
    try {
      ...
      if(this.account.ensureMode)await this.account.ensureMode(researchRuntime?'research':'normal');
      ...
      if (config.web_search !== (researchRuntime?'live':'disabled') || config.default_permissions !== 'travel_preview' || config.model_provider !== 'openai'
        || Object.keys(config.mcp_servers || {}).length || config.project_doc_max_bytes !== 0
...
module.exports = { CodexEditor, OUTPUT_SCHEMA, decodeAnswer };
