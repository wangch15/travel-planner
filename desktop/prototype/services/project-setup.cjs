...
const { TRUSTED_FILES, regularBytes, sameTrusted, trustedFile, trustedFilter } = require('./backup.cjs');
  ...
  async installTrustedHooks(root) {
    try {
      ...
      for (const relative of TRUSTED_FILES) {
        ...
        if (!trustedFile(relative, await fs.readFile(file), await fs.readFile(path.join(this.trustedRoot, relative)))) return false;
      }
