...
test('已經是最新：講一句就好，不要嚇人', () => {
  const out = renderReport({ local: '1.3.0', upstream: '1.3.0', entries: [] });
...
test('全部都不需要 migrate 時要講明，不要讓人以為有風險', () => {
  ...
  const out = renderReport({ local: '1.2.1', upstream: '1.2.1', entries: [] });
  assert.match(out, /已經是最新/);
});
...
test('模板作者本人：origin 就是上游，不該報「remote 沒設好」', () => {
  const out = renderReport({ local: '1.1.0', upstream: '1.1.0', entries: [], remote: 'origin' });
