...
**你不需要懂程式，也不需要先裝任何東西。** 你只要複製下面那段話貼給你的 AI 助手，剩下它會處理。
...
到 [github.com](https://github.com) 註冊。免費。只是用來放你的行程資料。

### 2️⃣ 把帳號名稱給我

我把你加進這個專案，你才看得到這一頁。（你現在看得到，代表這一步已經完成了。）

### 3️⃣ 複製下面整段，貼給你的 AI 助手

Claude Code、Codex、Gemini CLI 都可以。點程式碼框右上角的複製按鈕：

```text
我想做一個旅程網頁。請用這個模板：
https://github.com/wangch15/travel-planner

我不懂程式，電腦上也沒有設定過 git 或任何開發環境。
請你從頭到尾幫我處理，包含安裝工具、下載這個專案、設定帳號連線。

需要我本人操作的步驟（註冊帳號、在瀏覽器上按同意授權），
...
> 這個 repo 是私有的，我已經被加為協作者。請先跑 `gh auth login` 讓我在瀏覽器上授權，然後再試一次。
...
代表你還沒被加進專案，或還沒登入 GitHub。先在瀏覽器登入 GitHub，再點一次連結。這一頁在瀏覽器上就看得到，不需要先裝任何東西。
...
跟你的 AI 說就好，例如「第三天的午餐換一家」「加一個景點」。它知道要改哪裡，改完會再讓你看過才更新網站。
