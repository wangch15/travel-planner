...
> **還沒準備好環境？**（沒裝過 Node、git，或還沒辦 Cloudflare 帳號）
> 回頭跟給你這個連結的人要「前置準備」那段，那裡有一段 prompt 會帶你把帳號與環境一次弄好。
...
要邀請朋友來用的話，看 [`HELPER.md`](HELPER.md)——裡面有一段可以直接複製、用 LINE 或 email 傳給他們的訊息，涵蓋辦帳號與環境準備，**不需要他先有這個 repo 的權限**。
