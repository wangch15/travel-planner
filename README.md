...
| 平台 | 要做什麼 | 什麼時候要 |
|---|---|---|
| [GitHub](https://github.com) | 註冊，把帳號名稱給我 | 現在（你看得到這一頁，代表已經完成了） |
| [Cloudflare](https://dash.cloudflare.com/sign-up) | 註冊就好，其他不用設定 | 最後上線時才用到 |

兩個都免費，不用填信用卡。**這兩個只能你本人辦**——要收驗證信。

### 2️⃣ 讓你的 AI 準備好電腦環境

**你不用先裝任何東西。** 把這段貼給你的 AI，缺什麼它會幫你裝：

```text
我要用一個工具做旅程網頁，但我電腦上沒有裝過任何開發相關的東西。
...
補充資訊：
- 我的作業系統是：（填 Windows 或 Mac）
- 我的 GitHub 帳號是：（填你的帳號名稱）
...
需要用**可以在你電腦上執行指令**的 AI，例如 Claude Code、Codex CLI、Gemini CLI。
一般的網頁版聊天機器人做不了這件事。

### 3️⃣ 環境好了之後，複製下面整段貼給你的 AI
...
> **哪些一定要你本人做？** 只有四件：辦帳號、接受協作者邀請、在瀏覽器按「同意授權」、安裝 AI 助手本身。
> 其他——裝 Node／git／gh、設定 git、下載專案、產地圖、抓照片、部署——**全部可以交給 AI**。
...
要邀請朋友來用的話，[`docs/invite.md`](docs/invite.md) 有一段可以直接複製、用 LINE 或 email 傳給他們的訊息，含要辦哪些帳號與環境準備的 prompt。
