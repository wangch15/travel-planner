...
- **網站圖示改成 Travel Planner 標誌。** 瀏覽器分頁與加到手機主畫面時顯示 Travel Planner 圖示（依系統亮／暗色切換），不再用 `theme.favicon` 的 emoji；舊設定留著不會出錯。
- **推到公開模板前多擋兩種私人資料。** pre-push 以前只擋 trips/ 底下非底線開頭的資料夾，現在除了 `_example` 以外都擋，包括桌面 App 封存的 `trips/_archived/` 與 `trips/_profile.md`。
- 資料需要 migrate：否。已經有專案的人，更新專案後重新發布網站就會換成新圖示。
