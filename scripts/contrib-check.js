...
const { BLOCKED, classifyPaths, scanHistory } = require('./lib/contribution-history.js');

const DEFAULT_BASE = 'upstream/main';

function renderVerdict({ blocked, engine, violations = [] }) {
  if (blocked.length) {
    const lines = ['✗ 這個分支不能開 PR——裡面有不該公開的東西：', ''];
    for (const p of blocked) {
      const why = (BLOCKED.find((b) => b.re.test(p)) || {}).why || '';
      const commit = violations.find((v) => v.path === p)?.commit;
      lines.push(`  ${p}${why ? `　（${why}）` : ''}${commit ? `　commit ${commit}` : ''}`);
    }
    lines.push('');
...
function changedPaths(base) {
  return scanHistory(base, 'HEAD', { root: ROOT }).paths;
}
...
if (require.main === module) {
  ...
  try {
    const verdict = scanHistory(base, 'HEAD', { root: ROOT });
